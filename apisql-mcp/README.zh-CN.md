...
一个通用的数据库 MCP（Model Context Protocol）服务器，通过 [apiSQL](https://www.apisql.cn) 平台让 AI 助手能够执行 SQL 查询。同时连接 MySQL、PostgreSQL、SQL Server、Oracle 等多种数据库。
...
- **完整 SQL 支持** —— 全面支持 DDL（CREATE、ALTER、DROP）和 DML（SELECT、INSERT、UPDATE、DELETE）操作，执行存储过程和函数UDF。
- **可观测性** —— 通过 apiSQL 平台提供内置日志记录和审记。
